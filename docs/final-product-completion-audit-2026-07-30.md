...
Current PR head after the latest cart-image fallback guard: `fa41c6ba50d2475c9989ab70ad3ff2fc2c15b7f6`.

Exact current PR head and check status are also recorded in the PR #31 body and newest checkpoint comment.
...
| GitHub/local audit loop | Latest pushed PR checkpoint is green. | GitHub workflow runs: 4 of 4 success on `fa41c6ba50d2475c9989ab70ad3ff2fc2c15b7f6`; exact SHA is in the PR body and newest checkpoint comment. Current patch: `git diff --check`, lint, typecheck, build, checkout fallback, store visuals, CI-shaped launch-critical lane, and safety guardrails passed. | Keep PR #31 green until a non-production preview is explicitly approved. |
