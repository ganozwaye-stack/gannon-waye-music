...
function TikTokDiagnosticsPanel() {
  ...
  const { data: authData } = useQuery({
    queryKey: ['tiktokAuthUrl'],
    queryFn: () => base44.functions.invoke('tiktokOAuth', { action: 'get_auth_url' }),
    retry: false,
  });

  const oauthUrl = authData?.data?.auth_url || '';
  let urlParams = {};
  try { urlParams = Object.fromEntries(new URL(oauthUrl).searchParams); } catch {}

  const rawKey = urlParams.client_key || '';
  const redirectUri = urlParams.redirect_uri || '';
  const scopes = urlParams.scope || '';
