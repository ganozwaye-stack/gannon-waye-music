...
import {
  ...
  RefreshCw, Clock, CreditCard, Package, Webhook,
      ...
      <Card className="border-blue-500/30 bg-blue-500/5">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-blue-400" /> Stripe Tax & GST Status
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-2 text-sm">
          {[
            { label: 'automatic_tax: { enabled: false }', ok: true, detail: 'Set in createCheckoutSession — line 253 (GST exempt under $75k threshold)' },
            { label: 'customer_creation: "always"', ok: true, detail: 'Set — Stripe saves customer for records' },
            { label: 'billing_address_collection: "required"', ok: true, detail: 'Set — required for billing calculations' },
          ...
          <div className="mt-3 border border-blue-500/20 bg-blue-500/10 rounded-lg p-3 text-xs">
            <p className="font-semibold text-blue-300 mb-1">GST & Tax Exemption Confirmed</p>
            <p className="text-muted-foreground">Since your business revenue is below the $75,000 AUD threshold, you are not registered for GST and are exempt from collection. Automatic tax calculations are disabled in your code to prevent Stripe API blocks. No additional actions are required in your Stripe Tax dashboard at this time.</p>
          </div>
        </CardContent>
      </Card>
