...
function addRealPhotoCylinder(scene, texture, {
  radius,
  height,
  position,
  opacity = 0.42,
  ...
  cylinder.name = name || 'real garden environment';
  cylinder.position.set(...position);
  cylinder.rotation.y = Math.PI * 0.22;
  cylinder.userData.realFoliage = true;
  cylinder.userData.basePosition = cylinder.position.clone();
  cylinder.userData.baseRotation = cylinder.rotation.clone();
  cylinder.userData.motion = 0.004;
  scene.add(cylinder);
  return cylinder;
}
...
function addRealGardenLayers(scene, textureLoader) {
  ...
  const fernGullyCanopy = loadRealTexture(textureLoader, GARDEN_TEXTURES.fernGullyCanopy);
  ...
  addRealPhotoCylinder(scene, fernGullyCanopy, {
    radius: 22,
    height: 12.5,
    position: [0, 4.7, -14],
    opacity: 0.42,
    name: 'Melbourne Fern Gully tall tree reference canopy',
  });

  [
    { width: 5.4, height: 1.75, position: [-4.75, 4.55, 10.7], rotation: [-0.16, 0.68, -0.04], opacity: 0.24 },
    { width: 5.2, height: 1.7, position: [4.75, 4.5, 2.2], rotation: [-0.12, -0.68, 0.03], opacity: 0.2 },
    { width: 5.6, height: 1.85, position: [-4.7, 4.2, -14.2], rotation: [-0.06, 0.7, -0.02], opacity: 0.18 },
...
function cameraPoint(progress) {
  const stops = [
    { p: 0, pos: [0, 3.65, 16], target: [0, 6.35, 3.6] },
    { p: 0.18, pos: [0, 2.28, 9.2], target: [0, 2.05, -1.5] },
    ...
    <section ref={sectionRef} id="garden-walk" className="relative h-[520vh] bg-[#020502] text-[#fff7df]">
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <img
          ...
          alt=""
          aria-hidden="true"
          className="absolute inset-x-0 top-0 z-0 h-[56%] w-full object-cover object-top opacity-28"
          style={{ filter: 'brightness(0.78) contrast(1.08) saturate(1.08)' }}
        />
        <img
          src={GARDEN_TEXTURES.suburbanTreeline}
          alt=""
          aria-hidden="true"
          className="absolute inset-x-0 bottom-[12%] z-0 h-[28%] w-full object-cover object-center opacity-22"
          style={{ filter: 'brightness(0.64) contrast(1.06) saturate(0.92)' }}
        />
