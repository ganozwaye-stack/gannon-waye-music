...
import { Heart, Lock, Plus, Play, Pause, Edit2, Trash2, Save, X, Upload } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';

// Lifted from src/pages/admin/coaching/MeditationLibrary.jsx — Meditation entity
// management, type/status labels and the private draft list.
const TYPE_LABELS = {
...
export default function CoachingMeditationsTab() {
  ...
  const togglePlay = (m) => setPlaying(playing === m.id ? null : m.id);
    ...
    <div className="space-y-6 pb-10">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex-1">
          <h2 className="text-xl font-display font-bold gradient-gold-text">Meditation &amp; Mindset Library</h2>
          <p className="text-sm text-muted-foreground">Private meditations — not published to clients</p>
        </div>
        <Button type="button" onClick={handleNew} size="sm" className="gap-2"><Plus className="w-4 h-4" />Add Meditation</Button>
      </div>
        ...
        <Card className="border-primary/30">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold">{editing.id ? 'Edit Meditation' : 'New Meditation'}</h3>
            ...
            <CardContent className="p-4 flex items-center justify-between gap-3 flex-wrap">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {m.notes && <p className="text-[11px] text-muted-foreground/40 mt-1">Notes: {m.notes}</p>}
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {m.audio_url ? (
                  <Button type="button" variant="ghost" size="sm" className="text-xs gap-1 text-muted-foreground hover:text-primary" onClick={() => togglePlay(m)}>
                ...
                <button type="button" onClick={() => handleEdit(m)} className="text-muted-foreground hover:text-primary"><Edit2 className="w-3 h-3" /></button>
                <button type="button" onClick={() => deleteMut.mutate(m.id)} className="text-muted-foreground hover:text-destructive"><Trash2 className="w-3 h-3" /></button>
              </div>
            </CardContent>
          </Card>
        ))}
