...
import { BookOpen, Edit2, Plus, ChevronRight, Save, X, Trash2 } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';

// Lifted from src/pages/admin/coaching/CoachingPrograms.jsx — CoachingOffer entity
// management, draft program definitions and the pricing tier builders.
const empty = {
...
export default function CoachingProgramsTab() {
    ...
    <div className="space-y-6 pb-10">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex-1">
          <h2 className="text-xl font-display font-bold gradient-gold-text">Programs &amp; Offers (Staging)</h2>
          <p className="text-sm text-muted-foreground">Draft coaching programs — private, not published</p>
        </div>
        <Button type="button" onClick={handleNew} size="sm" className="gap-2"><Plus className="w-4 h-4" />Add Program</Button>
      </div>
        ...
        <Card className="border-primary/30">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold">{editing.id ? 'Edit Program' : 'New Program'}</h3>
              <button type="button" onClick={() => setEditing(null)} className="text-muted-foreground hover:text-foreground"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="border border-border/40 rounded-lg p-3 space-y-2">
              ...
              {(form.what_we_cover || []).map((item, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  <button type="button" onClick={() => removeInclusion(i)} className="text-muted-foreground hover:text-destructive"><X className="w-3 h-3" /></button>
                </div>
              ))}
              <div className="flex gap-2">
                <input value={inclusionInput} onChange={e => setInclusionInput(e.target.value)} placeholder="Add inclusion (e.g. Weekly 60-min session)" className="flex-1 bg-background border border-border rounded px-2 py-1.5 text-xs" onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addInclusion(); } }} />
                <Button type="button" onClick={addInclusion} variant="outline" size="sm" className="text-xs">Add</Button>
              </div>
            </div>
            ...
            <div className="border border-border/40 rounded-lg p-3 space-y-2">
              ...
              {(form.what_they_leave_with || []).map((item, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  <button type="button" onClick={() => removeOutcome(i)} className="text-muted-foreground hover:text-destructive"><X className="w-3 h-3" /></button>
                </div>
              ))}
              <div className="flex gap-2">
                <input value={outcomeInput} onChange={e => setOutcomeInput(e.target.value)} placeholder="Add outcome" className="flex-1 bg-background border border-border rounded px-2 py-1.5 text-xs" onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addOutcome(); } }} />
                <Button type="button" onClick={addOutcome} variant="outline" size="sm" className="text-xs">Add</Button>
              </div>
            </div>

            <div className="flex items-center gap-2">
              <input type="checkbox" checked={form.is_published} onChange={e => setForm({ ...form, is_published: e.target.checked })} id="hub-program-pub" />
              <label htmlFor="hub-program-pub" className="text-xs text-muted-foreground">Published (visible on public site)</label>
            </div>
            <Button type="button" onClick={handleSave} size="sm" className="gap-2"><Save className="w-3 h-3" />Save</Button>
          </CardContent>
        </Card>
      )}
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button type="button" onClick={(e) => { e.stopPropagation(); handleEdit(p); }} className="text-muted-foreground hover:text-primary p-1"><Edit2 className="w-3 h-3" /></button>
                  <button type="button" onClick={(e) => { e.stopPropagation(); deleteMut.mutate(p.id); }} className="text-muted-foreground hover:text-destructive p-1"><Trash2 className="w-3 h-3" /></button>
