...
import { Film, Copy, RefreshCw, AlertTriangle } from 'lucide-react';

// Lifted from src/pages/admin/VideoAgentCommand.jsx — Phase 1 transcript analysis,
// hook detection and CapCut prompt generators, plus the AgentMessage history.
export default function VideoAgentPanel() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-xl font-display font-bold gradient-gold-text">Video Agent Command</h3>
      ...
      <div className="flex flex-wrap gap-2">
        {TABS.map(t => (
          <Button type="button" key={t.id} variant={tab === t.id ? 'default' : 'outline'} size="sm" onClick={() => setTab(t.id)}>{t.label}</Button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          {result?.analysis ? (
            <div className="space-y-3">
                  ...
                  {[
                    ...
                    <div key={label}>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <Button type="button" variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={() => copy(value)}><Copy className="w-3 h-3" /></Button>
                      </div>
                    </div>
                  ))}
                  ...
                  {result.analysis.capcut_prompt && (
                    <div>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <Button type="button" variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={() => copy(result.analysis.capcut_prompt)}><Copy className="w-3 h-3" /></Button>
                      </div>
                    </div>
                  )}
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => setResult(null)}>Analyse Another</Button>
            </div>
          ) : (
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <Button type="button" onClick={runAnalysis} disabled={running} className="w-full">
              ...
              {[
                ...
                <div key={label} className="border border-border/30 rounded-lg p-3 mb-2">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <Button type="button" variant="ghost" size="sm" className="h-6 text-xs" onClick={() => copy(prompt)}>Copy</Button>
                  </div>
