...
// Lifted from src/pages/admin/LyricsArchive.jsx — the private lyrics archive
// with version tabs, search, CSV export and the full edit form.
const TABS = [
...
export default function LyricsArchivePanel() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['lyrics'] }); setEditing(null); },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['lyrics'] }) },
  });
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <h3 className="font-display text-xl text-foreground">Lyrics Archive</h3>
        ...
        <div className="flex gap-2">
          <Button type="button" onClick={handleExport} variant="outline" className="gap-2"><Download className="w-4 h-4" /> Export</Button>
          <Button type="button" onClick={handleNew} className="gap-2"><Plus className="w-4 h-4" /> Add Lyrics</Button>
        </div>
      </div>
      ...
      {editing && (
        <div className="mb-6 bg-card border border-border rounded-xl p-5 space-y-3">
          ...
          <Button type="button" onClick={handleSave} className="gap-2"><Save className="w-4 h-4" /> Save</Button>
        </div>
      )}
          ...
          <div key={lyric.id} className="bg-card/50 border border-border/40 rounded-xl p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex gap-2 shrink-0">
                <button type="button" onClick={() => handleEdit(lyric)} className="text-muted-foreground hover:text-primary"><Edit2 className="w-4 h-4" /></button>
                <button type="button" onClick={() => deleteMutation.mutate(lyric.id)} className="text-muted-foreground hover:text-destructive"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        ))}
