...
// Lifted from src/pages/admin/ImageEditor.jsx — canvas editor with FILTERS,
// STICKERS and TRANSITIONS configs, brightness/contrast sliders, layers and
// history. Cloud save uses UploadPublicFile (the available integration).
const FILTERS = {
  none: 'none',
...
export default function ImageEditorPanel() {
  ...
  }, [history, historyIndex, brightness, contrast, saturation, hue, blur, rotation, scale, filter, layers]);
  ...
  const handleUploadToCloud = async () => {
    ...
    canvas.toBlob(async (blob) => {
      try {
        const { file_url } = await base44.integrations.Core.UploadPublicFile({ file: blob });
  ...
  return (
    <div className="flex flex-col bg-background rounded-xl border border-border/40 overflow-hidden">
      {/* Header */}
      <div className="border-b border-border/40 px-4 py-3 flex items-center justify-between bg-card/50 flex-wrap gap-2">
        <div className="flex items-center gap-4">
          <h3 className="font-display text-lg text-foreground">Pro Image Editor</h3>
          <div className="flex items-center gap-2">
            <Button type="button" variant="ghost" size="icon" onClick={undo} disabled={historyIndex <= 0}>
              <Undo className="w-4 h-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" onClick={redo} disabled={historyIndex >= history.length - 1}>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button type="button" variant="outline" size="sm" onClick={handleDownload} disabled={!image} className="gap-2">
            <Download className="w-4 h-4" /> Export
          </Button>
          <Button type="button" size="sm" onClick={handleUploadToCloud} disabled={!image} className="gap-2 gradient-gold-button border-0">
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        {/* Left Sidebar - Tools */}
        <div className="lg:w-64 border-b lg:border-b-0 lg:border-r border-border/40 bg-card/30 overflow-hidden">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
            <TabsList className="w-full justify-start rounded-none border-b border-border/40 bg-transparent p-0 h-auto flex flex-wrap">
            ...
            <ScrollArea className="flex-1 p-4">
              <TabsContent value="adjust" className="space-y-4">
                ...
                <Button type="button" variant="outline" size="sm" onClick={resetAdjustments} className="w-full">Reset All</Button>
              </TabsContent>
              ...
              <TabsContent value="text" className="space-y-4">
                ...
                <Button type="button" onClick={addText} className="w-full">Add Text</Button>
              </TabsContent>
              ...
              <TabsContent value="layers" className="space-y-2">
                {layers.map((layer, idx) => (
                  <div key={layer.id} className="flex items-center gap-2 p-2 bg-secondary/30 rounded-lg">
                    ...
                    <Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => removeLayer(layer.id)}>
        ...
        {/* Canvas Area */}
        <div className="flex-1 bg-black/50 flex items-center justify-center p-8 min-h-[420px]" ref={containerRef}>
          {image ? (
            <canvas ref={canvasRef} className="max-w-full max-h-[600px] shadow-2xl" />
          ) : (
            <div
        ...
        <div className="lg:w-48 border-t lg:border-t-0 lg:border-l border-border/40 bg-card/30 p-4 space-y-4">
          <div>
            <Label className="font-body text-xs mb-2 block">Quick Crop</Label>
            <div className="grid grid-cols-3 gap-2">
              {[1, 1.5, 0.75].map(ratio => (
                <Button type="button" key={ratio} variant="outline" size="sm" className="text-xs">
                  {ratio}:1
                </Button>
              ))}
            </div>
          ...
          <div>
            ...
            {TRANSITIONS.map(t => (
              <Button type="button" key={t.name} variant="outline" size="sm" className="w-full justify-start text-xs mb-1">
          ...
          <div className="pt-4 border-t border-border/30">
            ...
            <div className="space-y-2">
              <Button type="button" variant="outline" size="sm" className="w-full justify-start text-xs">
