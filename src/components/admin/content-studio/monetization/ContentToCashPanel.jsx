...
import {
  Zap, Video, DollarSign, TrendingUp, ChevronRight,
  Loader2, Music, Play
} from 'lucide-react';
import { toast } from 'sonner';

// Lifted from src/pages/admin/ContentToCash.jsx — revenue formats, pipeline
// stages and the AI brief generator.
...
export default function ContentToCashPanel() {
...
Keep it practical. Gannon will read this brief and film it himself. Make it feel natural, not scripted.`,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-display font-bold gradient-gold-text">Content-to-Cash Engine</h3>
      ...
      <div>
        <div className="flex flex-wrap gap-2 mb-4">
          {['pipeline', 'brief', 'formats', 'revenue'].map(t => (
            <button type="button" key={t} onClick={() => setTab(t)}
              className={`font-body text-xs px-3 py-1.5 rounded-lg border transition-all ${tab === t ? 'bg-primary/10 border-primary text-primary' : 'border-border/50 text-muted-foreground hover:border-primary/40'}`}>
              {t === 'pipeline' ? `Pipeline (${videos.length})` : t === 'brief' ? 'Brief Generator' : t === 'formats' ? 'Formats' : 'Revenue Attribution'}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-3">
            ...
            {videos.length === 0 ? (
              <div className="text-center py-12 border border-dashed border-border rounded-xl">
                ...
                <Button type="button" variant="outline" className="mt-3 text-xs" onClick={() => setTab('brief')}>Generate First Brief</Button>
              </div>
            ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {v.platform_url && (
                      <a href={v.platform_url} target="_blank" rel="noreferrer">
                        <Button type="button" size="sm" variant="outline" className="text-xs gap-1"><Play className="w-3 h-3" />View</Button>
                      </a>
                    )}
                  </div>
                </div>
            ...
            <div className="flex gap-2">
              <Link to="/tiktok-platform-review"><Button type="button" variant="outline" className="text-xs gap-1"><Video className="w-3 h-3" />TikTok Draft Studio</Button></Link>
              <Link to="/admin/social-schedule-queue"><Button type="button" variant="outline" className="text-xs gap-1"><Zap className="w-3 h-3" />Social Content Gen</Button></Link>
            </div>
          </div>
        )}
        ...
        {tab === 'brief' && (
          <div className="space-y-4">
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {CONTENT_FORMATS.map(f => (
                  <button type="button" key={f.id} onClick={() => setSelectedFormat(f.id)}
            ...
            <Button type="button" className="gradient-gold-button border-0 gap-2" onClick={generateBrief} disabled={generating}>
              ...
              <div className="space-y-3 border border-primary/20 rounded-xl p-5">
                ...
                <div className="flex gap-2 pt-2">
                  <Link to="/tiktok-platform-review">
                    <Button type="button" size="sm" className="gradient-gold-button border-0 text-xs gap-1"><Video className="w-3 h-3" />Go to TikTok Studio</Button>
                  </Link>
                </div>
              </div>
            )}
            ...
            {CONTENT_FORMATS.map(f => (
              <div key={f.id} className="border border-border rounded-xl p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Button type="button" size="sm" variant="outline" className="text-xs shrink-0" onClick={() => { setSelectedFormat(f.id); setTab('brief'); }}>
                    Brief This
                  </Button>
                </div>
              </div>
