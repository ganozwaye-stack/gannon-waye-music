...
import {
  Bot, Brain, AlertTriangle, CheckCircle2, Clock,
  Zap, BookOpen, Loader2, Copy, Eye, Activity, Shield, Lightbulb
} from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import { toast } from 'sonner';

// Lifted from src/pages/admin/ContentDashboard.jsx — alerts/approvals, research
// feed, creative engine, bot activity, workflow map and the 12 interaction rules.
const BOT_RULES = [
...
const VIEWS = [
  { key: 'alerts', label: '🚨 Alerts & Approvals' },
  { key: 'research', label: '📚 Research Feed' },
  { key: 'creative', label: '💡 Creative Engine' },
  { key: 'activity', label: '⚡ Bot Activity' },
  { key: 'workflow', label: '🗺 Bot Workflow' },
  { key: 'rules', label: '📋 Interaction Rules' },
];

export default function ContentDashboardPanel() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h3 className="text-xl font-display font-bold gradient-gold-text">Content Dashboard</h3>
                ...
                {openAlerts.map(a => (
                  <Card key={a.id} className={`border ${a.severity === 'critical' ? 'border-red-500/30' : a.severity === 'high' ? 'border-orange-500/30' : 'border-primary/20'}`}>
                    <CardContent className="p-3 flex items-start gap-3">
                      ...
                      <Button type="button" size="sm" variant="ghost" className="text-xs h-7 shrink-0" onClick={() => dismissAlert.mutate({ id: a.id })}>Dismiss</Button>
                    </CardContent>
                  </Card>
                ))}
              </div>
                  ...
                  <Card key={item.id} className="border-primary/20">
                    <CardContent className="p-3">
                      ...
                      <div className="flex gap-2 mt-3">
                        <Button type="button" size="sm" className="bg-green-600 hover:bg-green-700 text-white h-7 text-xs" onClick={() => resolveApproval.mutate({ id: item.id, status: 'approved' })}>
                          <CheckCircle2 className="w-3 h-3 mr-1" /> Approve
                        </Button>
                        <Button type="button" size="sm" variant="destructive" className="h-7 text-xs" onClick={() => resolveApproval.mutate({ id: item.id, status: 'rejected' })}>
                          Reject
                        </Button>
                        <Button type="button" size="sm" variant="outline" className="h-7 text-xs" onClick={() => resolveApproval.mutate({ id: item.id, status: 'archived' })}>
                          Archive
                        </Button>
                      </div>
                    </CardContent>
        ...
        <div className="space-y-4">
          <Card className="border-primary/20 bg-primary/5">
            <CardContent className="p-4">
              ...
              <Button type="button" onClick={generateIdea} disabled={generatingIdea} className="gradient-gold-button">
            ...
            <Card className="border-primary/30">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button type="button" size="sm" variant="outline" onClick={() => { navigator.clipboard.writeText(freshIdea); toast.success('Copied!'); }}>
