...
// Lifted from src/pages/admin/SocialPostFactory.jsx — SPRINT_THEMES, the
// Metricool export generator, and the auto-save into ContentCalendarPost +
// ApprovalQueue. Metricool is blocked at UI level only; the token never
...
function CopyBtn({ text, label = 'Copy' }) {
  ...
  return (
    <button type="button" onClick={copy} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div key={f.key} className="bg-secondary/30 border border-border/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                <CopyBtn text={val} />
                {long && (
                  <button type="button" onClick={() => toggle(f.key)} className="text-muted-foreground hover:text-foreground transition-colors">
...
export default function PostFactoryPanel() {
...
Return JSON:
{
  ...
  "content_notes": "Production/posting notes"
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ['schedule-queue'] });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-display text-lg text-foreground">Social Post Factory</h3>
        <p className="font-body text-sm text-muted-foreground mt-0.5">Generate individual platform briefs · auto-saved to approval queue · Metricool export ready</p>
      </div>
      ...
      <Card className="border-primary/20">
        ...
        <CardContent className="space-y-4">
          ...
          <Button type="button" onClick={handleGenerate} disabled={generating} className="w-full gradient-gold-button border-0 gap-2">
            {generating
