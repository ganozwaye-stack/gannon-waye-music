...
import { CheckCircle2, XCircle, AlertTriangle, RefreshCw, ExternalLink, Shield, Wifi, Lock, Info, Copy, ClipboardList } from 'lucide-react';
import { toast } from 'sonner';

// Lifted from src/pages/admin/TikTokPlatformReviewAdmin.jsx — OAuth diagnostics,
// PORTAL_CHECKS checklist and the approval-gate notice.
...
export default function TikTokReviewPanel() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h3 className="font-display text-lg font-bold gradient-gold-text">TikTok Platform Review</h3>
          <p className="text-muted-foreground text-sm mt-1">OAuth diagnostics · client_key whitespace analysis · portal checklist</p>
        </div>
        <Button type="button" onClick={load} disabled={loading} variant="outline" className="gap-2">
        ...
        <CardContent className="p-4 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2">
            {!status?.connected
              ...
              : <Button type="button" onClick={handleDisconnect} variant="outline" size="sm" className="text-red-400 border-red-500/30 gap-1">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <a href="https://developers.tiktok.com/apps/" target="_blank" rel="noopener noreferrer">
          <Button type="button" size="sm" variant="outline" className="gap-1.5"><ExternalLink className="w-3.5 h-3.5" />TikTok Developer Portal</Button>
        </a>
        <a href="https://base44.com" target="_blank" rel="noopener noreferrer">
          <Button type="button" size="sm" variant="outline" className="gap-1.5"><Lock className="w-3.5 h-3.5" />Base44 Secrets</Button>
        </a>
      </div>
