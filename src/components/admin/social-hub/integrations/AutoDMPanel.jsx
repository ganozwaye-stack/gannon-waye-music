...
import { MessageCircle, Shield, AlertTriangle, CheckCircle2, Play, Pause, Users, TrendingUp, Copy, Plus } from 'lucide-react';
import { toast } from 'sonner';

// Lifted from src/pages/admin/InstagramAutoDMCommand.jsx — safety rules, safe
// copy templates, UTM links and the AgentActionProposal approval gate.
...
export default function AutoDMPanel() {
  ...
  // Campaigns are stored in AgentActionProposal with proposal_type = 'social_campaign'
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h3 className="text-lg font-display font-bold gradient-gold-text flex items-center gap-2">
            <MessageCircle className="w-5 h-5" /> Instagram Auto-DM Command
          </h3>
        ...
        <Button type="button" onClick={() => setShowNewCampaign(true)} className="gap-2">
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-base">Funnel Flow</CardTitle></CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-base">Tracking UTM Links</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {[
            ...
            <div key={link.label} className="flex items-center justify-between border border-border rounded-lg p-2 gap-2">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-muted-foreground">{link.label}</p>
                <code className="text-xs text-primary break-all">{link.url}</code>
              </div>
              <Button type="button" size="sm" variant="ghost" className="shrink-0" onClick={() => { navigator.clipboard.writeText('https://' + link.url); toast.success('Copied'); }}>
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-base">Safe Public Comment Reply Templates</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {SAFE_REPLY_TEMPLATES.map(t => (
            <div key={t.id} className="flex items-center justify-between border border-border rounded-lg p-3 gap-2">
              <p className="text-sm">{t.text}</p>
              <Button type="button" size="sm" variant="ghost" className="shrink-0" onClick={() => { navigator.clipboard.writeText(t.text); toast.success('Copied'); }}>
                <Copy className="w-3 h-3" />
              </Button>
            </div>
          ))}
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-base">Opening DM Templates</CardTitle></CardHeader>
        <CardContent className="space-y-2">
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-base">Link DM Templates</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {SAFE_LINK_DM_TEMPLATES.map(t => (
            <div key={t.id} className="flex items-start justify-between border border-border rounded-lg p-3 gap-3">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-muted-foreground mb-1">{t.label}</p>
                <p className="text-sm text-foreground/80 break-all">{t.text}</p>
              </div>
              <Button type="button" size="sm" variant="ghost" className="shrink-0" onClick={() => { navigator.clipboard.writeText(t.text); toast.success('Copied'); }}>
                <Copy className="w-3 h-3" />
              </Button>
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-base">Campaigns (Require Approval)</CardTitle></CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                <div key={c.id} className="border border-border rounded-lg p-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {c.status === 'pending_approval' && (
                      <>
                        <Button type="button" size="sm" className="bg-green-600 hover:bg-green-700 text-white text-xs"
                    ...
                    {c.status === 'approved' && (
                      <Button type="button" size="sm" className="bg-blue-600 hover:bg-blue-700 text-white text-xs"
      ...
      {showNewCampaign && (
        <Dialog open onOpenChange={() => setShowNewCampaign(false)}>
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            <DialogHeader><DialogTitle>New Instagram Auto-DM Campaign</DialogTitle></DialogHeader>
            <div className="space-y-4 mt-2">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {SAFE_REPLY_TEMPLATES.map(t => (
                    <button type="button" key={t.id} onClick={() => setNewCampaign(p => ({ ...p, public_reply_template: t.text }))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {SAFE_DM_TEMPLATES.map(t => (
                    <button type="button" key={t.id} onClick={() => setNewCampaign(p => ({ ...p, opening_dm_template: t.text }))}
                      className="text-xs px-2 py-0.5 bg-secondary rounded hover:bg-primary/20 transition-colors">{t.label}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {SAFE_LINK_DM_TEMPLATES.map(t => (
                    <button type="button" key={t.id} onClick={() => setNewCampaign(p => ({ ...p, link_dm_template: t.text }))}
                      className="text-xs px-2 py-0.5 bg-secondary rounded hover:bg-primary/20 transition-colors">{t.label}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2 border-t border-border">
                <Button type="button" onClick={() => createMutation.mutate(newCampaign)} disabled={!newCampaign.name || createMutation.isPending}>
                  Submit to Approval Queue
                </Button>
                <Button type="button" variant="outline" onClick={() => setShowNewCampaign(false)}>Cancel</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
