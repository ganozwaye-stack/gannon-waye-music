...
// Lifted from src/pages/admin/InstagramStoryStudio.jsx — SLIDES config and the
// 9:16 story preview with its caption copy generator.
const SLIDES = [
  {
    id: 'launch',
    label: '🔥 Store is OPEN',
...
function StoryPreview({ slide }) {
  ...
  return (
    <div
      ...
      style={{
        width: 320, height: 568, background: slide.bg, borderRadius: 24,
        padding: '32px 28px 36px',
        boxShadow: `0 0 60px ${slide.accent}22, 0 24px 80px rgba(0,0,0,0.8)`,
        border: `1px solid ${slide.accent}22`, flexShrink: 0,
      }}
    >
      <div style={{
      ...
      <div>
        ...
        <p style={{
          ...
          color: `${slide.accent}88`, fontWeight: 700, textTransform: 'uppercase', marginBottom: 8,
        }}>{c.eyebrow}</p>
        ...
        {c.sub && (
          <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: `${slide.accent}70`, letterSpacing: '0.08em', marginBottom: 16 }}>{c.sub}</p>
        )}

        <div style={{ width: 40, height: 2, background: slide.accent, borderRadius: 2, marginBottom: 16, opacity: 0.5 }} />

        {c.body && (
          <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.60)', lineHeight: 1.6, whiteSpace: 'pre-line' }}>{c.body}</p>
        )}
  ...
  return (
    <div className="bg-card/40 border border-border/30 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <Button type="button" variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(caption); toast({ title: 'Caption copied!' }); }}>
...
export default function StoryStudioPanel() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-display text-lg text-foreground">Instagram Story Studio</h3>
        <p className="font-body text-sm text-muted-foreground/60 mt-0.5">Ready-to-post story slides to ignite excitement for the merch store</p>
      </div>
