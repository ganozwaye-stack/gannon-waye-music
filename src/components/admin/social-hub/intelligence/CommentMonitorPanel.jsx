...
// Lifted from src/pages/admin/SocialMonitor.jsx — CLASS_CONFIG and the comment
// triage rules, with archived sessions read from KnowledgeVault.
const CLASS_CONFIG = {
...
export default function CommentMonitorPanel() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h3 className="text-lg font-display font-bold gradient-gold-text">Social Comment Monitor</h3>
      ...
      <div className="flex gap-3 flex-wrap">
        <Button type="button" onClick={() => runTriage(true)} disabled={loading} className="gradient-gold-button">
        ...
        <Card>
          <CardContent className="p-4 space-y-3">
            ...
            <Textarea
              ...
              rows={5}
            />
            <Button type="button" onClick={() => runTriage(false)} disabled={loading || !manualComments} size="sm" className="gradient-gold-button">
                    ...
                    <div className="flex-1">
                      ...
                      {item.auto_response && (
                        <div className="mt-3 bg-secondary/50 rounded-lg p-3">
                          ...
                          <Button type="button" size="sm" variant="ghost" className="mt-2 h-6 text-xs" onClick={() => copyResponse(item.auto_response)}>
                    ...
                    <div className="flex-1">
                      ...
                      {item.auto_response && (
                        <div className="flex items-center gap-2">
                          ...
                          <Button type="button" size="sm" variant="ghost" className="h-6 text-xs shrink-0" onClick={() => copyResponse(item.auto_response)}>
      ...
      {recentTriage.length > 0 && !ran && (
        <div>
          <h4 className="text-base font-semibold mb-3">Previous Triage Sessions</h4>
          <div className="space-y-2">
            {recentTriage.map(item => (
              <div key={item.id} className="border border-border rounded-lg p-3 flex items-center justify-between gap-3">
                ...
                <Badge className="bg-secondary text-secondary-foreground text-xs shrink-0">archived</Badge>
              </div>
            ))}
          </div>
        </div>
