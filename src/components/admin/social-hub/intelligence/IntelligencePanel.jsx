...
// Lifted from src/pages/admin/SocialIntelligence.jsx — platform safety rule,
// intelligence prompts, SuperfanProfile tracker and ViralOpportunity feed.
...
export default function IntelligencePanel() {
  ...
  const VIEWS = [
    { key: 'analysis', label: 'Intelligence Analysis' },
    { key: 'superfans', label: `Superfan Tracker (${superfans.length})` },
    { key: 'viral', label: `Viral Ops (${viralOps.length})` },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-display font-bold gradient-gold-text">Social Intelligence</h3>
      ...
      <div className="border border-primary/30 bg-primary/5 rounded-lg p-3 flex items-center gap-3">
        ...
        <p className="text-primary text-xs"><strong>Platform Safety Rule:</strong> No automated engagement, mass following, DM spam, fake interactions, or paid follower purchases. All outreach suggestions require human approval before action.</p>
      </div>
          ...
          {ANALYSIS_CARDS.map(card => (
            <Card key={card.key}>
              ...
              <CardContent className="space-y-3">
                <Button type="button" size="sm" variant="outline" className="w-full" onClick={() => runAnalysis(card.key)} disabled={loading === card.key}>
      ...
      {view === 'superfans' && (
        <div className="space-y-4">
          <Button type="button" onClick={() => setShowSuperfanForm(true)} size="sm" className="gradient-gold-button border-0"><Plus className="w-4 h-4 mr-1" />Add Superfan</Button>
          {superfans.length === 0 ? (
          ...
          <Dialog open={showSuperfanForm} onOpenChange={setShowSuperfanForm}>
            <DialogContent className="max-w-sm">
              ...
              <div className="space-y-3">
                ...
                <Button type="button" className="w-full gradient-gold-button border-0" disabled={!superfanForm.username} onClick={() => createSuperfanMutation.mutate(superfanForm)}>Save Profile</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>
      ...
      {view === 'viral' && (
        viralOps.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-border rounded-xl">
            ...
            <p className="text-muted-foreground">Run "Viral Opportunity Scan" in the Analysis view to populate this feed.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {viralOps.map(op => (
