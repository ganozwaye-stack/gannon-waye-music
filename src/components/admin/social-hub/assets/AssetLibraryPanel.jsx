...
// Lifted from src/pages/admin/SocialAssetLibrary.jsx — ASSET_TYPES config,
// status badges, preview handling, upload form and CSV manifest import.
...
function AssetPreview({ asset, typeConf }) {
  ...
  return (
    <div className="relative aspect-[4/3] overflow-hidden rounded-lg border border-border/50 bg-black/40">
      {canPreview && isVideo ? (
        <video src={previewSrc} className="h-full w-full object-cover" muted playsInline preload="metadata" controls onError={() => setFailed(true)} />
      ) : canPreview && isImage ? (
        <img src={previewSrc} alt={asset.name || label} className="h-full w-full object-cover" loading="lazy" onError={() => setFailed(true)} />
      ) : (
...
function AssetCard({ asset, onUpdate, onDelete }) {
  ...
  return (
    <div className={`border rounded-xl p-4 hover:border-primary/40 transition-all bg-card/60 space-y-3 ${isSensitive ? 'border-red-500/40 bg-red-500/5' : 'border-border/50'}`}>
      ...
      {asset.notes && <p className="font-body text-xs text-muted-foreground/70 line-clamp-2">{asset.notes}</p>}
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <button type="button" onClick={() => onDelete(asset.id)} className="ml-auto text-muted-foreground/40 hover:text-destructive transition-colors">
  ...
  return (
    <Card className="border-primary/20">
      ...
      <CardContent className="space-y-3">
        ...
        <Button type="button" onClick={handleUpload} disabled={uploading} className="w-full gradient-gold-button border-0 gap-2">
...
function CsvImportPanel({ onImported }) {
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (ev) => setPreview(parseCsvManifest(ev.target.result));
      ...
      <CardContent className="space-y-3">
        ...
        {preview && (
          <div>
            ...
            <Button type="button" onClick={handleImport} disabled={importing} className="w-full mt-3 gradient-gold-button border-0 gap-2">
...
export default function AssetLibraryPanel() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-display text-lg text-foreground">Social Asset Library</h3>
        <p className="font-body text-sm text-muted-foreground mt-0.5">Upload, tag, preview, and manage footage, thumbnails, templates, and campaign assets</p>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button type="button" onClick={() => setShowCsvImport(v => !v)} className="font-body text-xs text-muted-foreground hover:text-primary flex items-center gap-1.5 transition-colors">
