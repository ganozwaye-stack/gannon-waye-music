...
// Lifted from src/pages/admin/MetricoolMediaPipeline.jsx — public URL
// normalization into a Metricool mediaId, with sensitive/raw assets blocked.
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {['image', 'video'].map(t => (
                <button type="button" key={t} onClick={() => setMediaType(t)}
          ...
          <div className="flex gap-2">
            <Button type="button" onClick={run} disabled={loading || !url} className="flex-1 gradient-gold-button border-0 gap-2">
...
export default function MediaPipelinePanel() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-display text-lg text-foreground">Metricool Media Pipeline</h3>
        <p className="font-body text-sm text-muted-foreground mt-0.5">Normalize approved assets → Metricool mediaId · Sensitive assets blocked by default</p>
      </div>
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="font-body text-xs text-muted-foreground mt-0.5">
            ...
            Use the <strong>Asset Library</strong> tab to verify and approve assets first.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 border border-dashed border-border/50 rounded-xl">
          ...
          <p className="font-body text-sm text-muted-foreground">No assets found. Upload in the Asset Library tab first.</p>
        </div>
      ) : (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    ...
                    {mediaId && <Badge className="bg-primary/10 text-primary border-0 text-[9px]">mediaId: {mediaId.slice(0, 8)}…</Badge>}
                  </div>
                  <p className="font-body text-xs text-muted-foreground">{asset.asset_type} · {asset.platform_tags?.join(', ') || 'all platforms'}</p>
                </div>
                <div className="shrink-0 flex items-center gap-2">
                  {asset.status !== 'raw' && !mediaId && <span className="font-body text-[10px] text-primary">Normalize →</span>}
