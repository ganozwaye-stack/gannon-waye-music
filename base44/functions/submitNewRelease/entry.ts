  ...
  try {
    ...
    if (body.private_draft_acknowledged !== true) {
      return Response.json({
        error: 'Confirm that this creates a private record and review shells only.',
      }, { status: 400 });
    }
    ...
    // These are intentionally empty private workspaces. The owner chooses
    // what to write or paste, then every later use stays separately controlled.
    const pressDraft = await sr.entities.ContentStudioRecord.create({
      ...
      release: title,
      caption: '',
      description: 'Private review shell. Write or paste approved press copy before any use.',
    ...
    const pitchDraft = await sr.entities.ContentStudioRecord.create({
      ...
      release: title,
      caption: '',
      description: 'Private review shell. Write or paste an approved pitch before any use.',
      approval_status: 'draft',
    });
    ...
    const emailDraft = await sr.entities.ContentStudioRecord.create({
      ...
      release: title,
      caption: '',
      description: 'Private review shell. Write or paste approved email copy before any use.',
      approval_status: 'draft',
    });
    ...
    return Response.json({
      ...
      pack: {
        press_release: false,
        playlist_pitch: false,
        subscriber_email: false,
        social_count: 0,
        lyric_quote: false,
        manychat: false,
        review_shells: 3,
