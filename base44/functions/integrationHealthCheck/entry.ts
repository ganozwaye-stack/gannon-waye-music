...
const PRINTFUL_ALLOWED_SETUP_SCOPES = new Set([
  'orders/read',
  'sync_products',
  'file_library',
  'webhooks',
...
function normalizePrintfulScopes(body) {
  const rawScopes = body?.result?.scopes;
  if (!Array.isArray(rawScopes)) return [];

  return [...new Set(rawScopes
    .map((entry) => typeof entry === 'string' ? entry : entry?.scope)
    .filter(Boolean))].sort();
}
...
async function runPrintfulReadOnlyCheck() {
  ...
  const scopesResponse = await fetchPrintfulJson('https://api.printful.com/oauth/scopes', token);
  let storesEndpoint = 'https://api.printful.com/v2/stores';
  let storesResponse = await fetchPrintfulJson(storesEndpoint, token);

  // Read-only compatibility fallback if the v2 store endpoint is unavailable.
  if (!storesResponse.ok && [404, 405].includes(storesResponse.status)) {
    storesEndpoint = 'https://api.printful.com/stores';
    storesResponse = await fetchPrintfulJson(storesEndpoint, token);
  }

  const scopes = normalizePrintfulScopes(scopesResponse.body);
  const stores = normalizePrintfulStores(storesResponse.body);
  const unexpectedScopes = scopes.filter((scope) => !PRINTFUL_ALLOWED_SETUP_SCOPES.has(scope));
  ...
  return {
    ...
    scopes,
    required_setup_scopes_present: {
      'orders/read': scopes.includes('orders/read'),
      sync_products: scopes.includes('sync_products'),
      file_library: scopes.includes('file_library'),
      webhooks: scopes.includes('webhooks'),
    },
    ...
    verification_limit: 'A single visible store strongly supports store-restricted access, but it cannot prove the token client type if the Printful account itself contains only one store.',
  };
}
...
Deno.serve(async (req) => {
  try {
    ...
    // to Printful and returns without running the normal health check or writing records.
